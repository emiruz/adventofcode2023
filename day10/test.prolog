...
connect(q(X0,Y0,63),q(X,Y,_)) :-
    once(X0\=X;Y0\=Y),
    1 >= sqrt((X-X0)^2 + (Y-Y0)^2),
    \+ (p(A,B,_),
	(Y0=Y,A<max(X,X0),A>min(X,X0);X0=X,B<max(Y,Y0),B>min(Y,Y0))).
connect(q(X0,Y0,46),q(X,Y,_)) :-
    once(X0\=X;Y0\=Y),
    1 >= sqrt((X-X0)^2 + (Y-Y0)^2).
...
fill(Ms,Fill) :-
    member(M,Ms),q(A,B,C),
    connect(M,q(A,B,C)),
    \+ memberchk(q(A,B,C),Ms),!,
    fill([q(A,B,C)|Ms],Fill).
fill(Ms,Ms).
...
solve(File,Part1,Part2) :-
    ...
    writeln(["Point count",PointCount]),

    X1=1,Y1=1,q(X1,Y1,46), fill([q(X1,Y1,46)],Fill),
    aggregate_all(count,member(q(_,_,46),Fill),Part2).
